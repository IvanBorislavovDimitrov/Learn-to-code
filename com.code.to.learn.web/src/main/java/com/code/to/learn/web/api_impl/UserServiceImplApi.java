...
public class UserServiceImplApi implements UserServiceApi {
	...
	private ResponseEntity<?> getBadRequestResponseEntity(UserException exception) {
		ErrorResponse.Builder message = new ErrorResponse.Builder().code(HttpStatus.BAD_REQUEST.value())
				.type(exception.getClass().getSimpleName()).message(exception.getMessage());
		return ResponseEntity.badRequest().body(parser.serialize(message));
	}
